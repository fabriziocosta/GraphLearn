...
class CoreInterfacePair:
    ...
    def __str__(self):
        return 'cip: int:%d, cor:%d, rad:%d, thi:%d, rot:%d' % \
               (self.interface_hash, self.core_hash,
                self.radius, self.thickness, self.root)
...
def graph_hash(graph, node_name_label=lambda id, node: node['hlabel']):
    ...
    """
    node_names = {n: calc_node_name(graph, n,  node_name_label)
                  for n in graph.nodes()}
    #tmp_fast_hash = lambda a, b: fast_hash([(a ^ b) + (a + b), min(a, b), max(a, b)])
    tmp_hash = lambda a, b: hash((min(a, b), max(a, b)))
    l = [tmp_hash(node_names[a], node_names[b])
         for (a, b) in graph.edges()]  # was tmp fast hash before
    l.sort()
...
def calc_node_name(interfacegraph, node, node_name_label=lambda id, node: node['hlabel']):
    ...
    l = [node_name_label(nid, interfacegraph.node[nid]) +
         dis for nid, dis in d.items()]
    l.sort()
    return hash(tuple(l))
...
def extract_core_and_interface(root_node=None,
    ...
    dist = {a: b for (a, b) in short_paths(graph,
                                           root_node if isinstance(root_node, list) else [
                                               root_node],
                                           thickness + radius)}
...
def _finalize_cip(root_node, graph, radius, thickness, dist, core_nodes, interface_nodes):
    ...
    test = lambda idd: 2 == sum(
        [neigh in core_nodes for neigh in graph.neighbors(idd)])
    mv_to_core = {
        idd: 0 for idd in interface_nodes if "edge" in graph.node[idd] and test(idd)}
    if mv_to_core:
    ...
    for no in interface_nodes:
        cip_graph.node[no][ddl] = cip_graph.node[
            no]['hlabel'] + dist[no] - (radius + 1)
...
def find_all_isomorphisms(home, other):
    try:
        if iso.faster_could_be_isomorphic(home, other):
            ...
            graph_label_matcher = iso.GraphMatcher(
                home, other, node_match=label_matcher)
            for index, mapping in enumerate(graph_label_matcher.isomorphisms_iter()):
                if index == 5:
                    logger.debug(
                        'lsgg_compose_util i checked more than 5 isomorphisms')
                yield mapping
        else:
...
def core_substitution(graph, orig_cip, new_cip):
    ...
    # get isomorphism
    iso = next(find_all_isomorphisms(
        orig_cip.interface_graph, new_cip.interface_graph))
    #iso = next(find_all_isomorphisms(orig_cip.interface_graph, new_cip.interface_graph))
    if len(iso) != len(orig_cip.interface_graph):
        logger.log(
            5, "lsgg_compose_util grammar hash collision, discovered in 'core_substution' ")
        return None

    # make graph union (the old graph and the new cip are now floating side by
    # side)
