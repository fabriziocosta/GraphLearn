...
class estimator:
    ...
    '''
    def fit(self,Graph_iter,vectorizer,nu=.5,n_jobs=-1):
        X = vectorizer.transform(Graph_iter)
        esti = self.fit_estimator(X, n_jobs)
        cesti = self.calibrate_estimator(X,esti,nu)
        return cesti
    ...
    def calibrate_estimator(self, X,estimator, nu=.5):
        '''
