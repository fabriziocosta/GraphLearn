...
from graphlearn01.minor.rna_graphmanager import get_sequence, getsucc, post

import graphlearn01.compose
import graphlearn01.decompose as gt
import graphlearn01
    ...
    for n,d in abstract_graph.nodes(data=True):
        if d['label']=='B':
            ...
            for a,b in getpairs(abstract_graph,mergelist,n):
                ...
                graphlearn01.compose.merge(abstract_graph, a, b)
...
def direct_abstractor(graph,v):
    ...
    n,not_used= graphlearn01.minor.rnaabstract.get_start_and_end_node(graph)
    tasks=[n]
    result = nx.Graph()
...
class PostProcessor:
    ...
    def rna_refold(self, digraph=None, seq=None,vectorizer=None):
        ...
        ex_di_graph = graphlearn01.minor.rnaabstract.expanded_rna_graph_to_digraph(expanded_graph)
